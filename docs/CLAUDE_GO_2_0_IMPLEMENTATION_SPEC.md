...
| `staff` | 已绑定微信的门店工作人员 | 唯一绑定门店的审核与核销 |
...
- 一个工作人员身份只能绑定一个门店，staff token 中的 `store_id` 必须来自服务端绑定关系。
- 小程序 staff 页面和 store staff 接口都不得提供门店选择、门店切换或重新绑定门店能力。
- v2 数据模型必须保证同一微信用户/同一 staff account 不会同时绑定多个门店；若从 v1 `staff` 表迁移时发现同一 `user_id` 多门店记录，必须进入迁移冲突清单，由业务人工确认保留哪一个绑定。
- 所有 staff 核销、积分审核、今日活动、核销历史查询都只使用该唯一绑定门店的 scope，前端传入的 `storeId` 不参与授权范围计算。
