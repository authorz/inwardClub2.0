...
| 门店公开信息 | `stores`,`settings`,`banner` | `/mini/stores`,`/{id}`,`/{id}/banners` | 重建/合并 | M0 | 已实现（列表/详情/banner） |
...
| 会员等级/充值档位/排行 | `vip_level`,`coin_product`,`recharge` | `/mini/membership-tiers`,`/recharge-products`,`/rankings` | 重建 | M1 | `/mini/*` 读（membership-tiers/recharge-products/rankings）已实现并文档化（`v2.yaml`）；`/rankings` 按 approved `point_savings` 汇总、支持 `week/month/all` 窗口（默认 all）；总后台 `/admin/membership-tiers`,`/admin/recharge-products` 增/改/禁用已实现（`v2.yaml`）；充值产品新增 `growthAmount` 字段（成长值发放量，默认 0），供充值结算发放成长值并驱动 VIP 升级（见"充值订单"行） |
| 签到/存取积分/提现/邀请 | `sign_in`,`save_points`,`points_withdrawal`,`invitations` | `/mini/sign-ins`,`/point-savings`,`/point-withdrawals`,`/invitations/*` | 重建 | M1 | 已实现并文档化（`v2.yaml`）：签到即时入账，梯度默认 day1..7=100..700、day7+ 封顶，由总后台 `/admin/rule-definitions`（ruleKey=`sign_in`，已种子化启用）可改值/禁用，无启用规则时回落服务端默认；存/取积分与提现落 `pending` 请求（幂等键声明）待门店后台审核；邀请绑定/列表已实现。其余积分规则值待业务确认（§13） |
...
| 充值订单 | `recharge` | `/mini/recharge-orders` | 重建 | M3 | 已实现：下单 + 结算入账；微信充值结算已在同一事务发放 coins（`amount + bonusAmount`，行为不变）+ growth 累积（`recharge_products.growth_amount`，默认 0，管理员可配）+ VIP 升级（按 `membership_tiers.threshold` 对成长值取最高档，仅升不降，写 `members.current_tier_id`）；各权益独立幂等键，重复通知不重复入账。待确认/待接线：降级/月度福利/补发、阈值口径（growth_value vs points）、线下聚合与消费渠道的 growth（见 acceptance 审计 §1.C 与 spec §13） |
...
| 微信支付回调（通用/活动/充值/点餐 4 套） | 4 个回调控制器 | `/internal/payments/wechat/notify` | 合并 | M3 | 已实现并文档化（`v2.yaml`）：单一回调 + 结算，按支付单区分业务；网关仍为 fake；会员充值单结算已在同一事务发放 coins/growth 并升级 VIP 档位（见"充值订单"行）；门店绑定订单结算会在同一事务写 `print:receipt` outbox（出小票）；非充值业务的规则化权益评估器已实现但均 config-gated、默认禁用——低消 `low_spend_reward` 由 worker `payment:post-process` 落地、邀请 `invite_reward` 由 `rule:post-process` 评估器承接；且微信结算侧目前不产生规则评估事件（仅线下聚合收款结算写 `payment:post-process`），其生产端仍待接线（见 acceptance 审计 §1.A/F） |
...
| 后台用户/员工/门店/运营配置 | `admins`,`staff`,`stores`,`settings` | `/admin/*`,`/store/*` | 重建 | M2 | 已实现：admin/store 账号（管理员/员工/收银员）、门店资料/设置、banner、打印机等运营配置；staff 单门店唯一约束已建表 |
| 规则引擎 | 分散写死逻辑 | `rule_definitions`,`rule_executions`,`benefit_grants` | 重建 | M2/M6 | CRUD 已实现并文档化（`/admin/rule-definitions/*`，`v2.yaml`）；规则评估读取侧已接线：低消 `low_spend_reward` 由 worker `payment:post-process` 落地发放、`benefit:vip-monthly` 与邀请 `invite_reward`（`rule:post-process`）由 `internal/modules/rule` 包评估；均 config-gated，除已种子化启用的 `sign_in` 外默认禁用（禁用即安全 no-op），具体权益值/资格待业务确认（见 acceptance 审计 §1.F） |
