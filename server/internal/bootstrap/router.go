...
func (a *App) registerMini(r *gin.Engine, mw *authn.Middleware) {
	...
	g.GET("/stores/:storeID/reservation-availability", a.storeLowSpendRuleHandler.MiniReservationAvailability)
...
func (a *App) registerAdminConsole(p *gin.RouterGroup) {
	...
	// Coupon template list read is registered on the shared admin read model.
...
func (a *App) registerStoreConsole(p, idem *gin.RouterGroup) {
	...
	// Coupon template list read is registered on the store-scoped admin read model.
