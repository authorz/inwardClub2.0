...
// GrantPurchasedCoupons issues one 30-day entitlement per purchased coupon
// product unit. The order-line snapshot and deterministic idempotency key make
// retries safe and preserve the entitlement selected when the order was made.
func GrantPurchasedCoupons(
	...
	rows, err := tx.QueryContext(ctx, `SELECT foi.id, foi.coupon_template_id_snapshot, foi.quantity
	...
	for rows.Next() {
		...
		if err := rows.Scan(&line.lineID, &line.templateID, &line.quantity); err != nil {
			rows.Close()
			return 0, apperr.Internal(err)
		}
