...
const pointSavingSelect = `SELECT ps.id, ps.store_id, ps.member_id,
	...
	ps.rule_version, ps.points_divisor, ps.below_base_points_divisor,
	ps.business_date, ps.business_start_at, ps.business_end_at,
...
func scanPointSaving(row interface{ Scan(...any) error }) (PointSaving, error) {
	...
	if err := row.Scan(&p.ID, &p.StoreID, &p.MemberID, &p.MemberName, &p.Phone, &p.MemberAvatarAssetID, &p.MemberAvatarURL, &p.StoreName,
		...
		&p.RuleVersion, &p.PointsDivisor, &p.BelowBasePointsDivisor,
		&p.BusinessDate, &p.BusinessStartAt, &p.BusinessEndAt,
...
func (r *storeSQLRepository) StaffTodayOperations(ctx context.Context, storeID int64, dayStart, dayEnd time.Time, operationType string, limit, offset int) (StaffTodayOperationsData, error) {
	...
	const pointSummary = `SELECT COUNT(*), COALESCE(SUM(points), 0) FROM %s
		WHERE store_id = ? AND status = 'approved' AND created_at >= ? AND created_at < ?`
	...
	const q = `SELECT record_key, operation_type, member_id, member_name, phone,
		...
		ORDER BY created_at DESC, record_key DESC LIMIT ? OFFSET ?`
	rows, err := r.db.QueryContext(ctx, q,
		storeID, dayStart, dayEnd,
		storeID, dayStart, dayEnd,
		storeID, dayStart, dayEnd,
		operationType, operationType, limit, offset,
	)
	if err != nil {
		return StaffTodayOperationsData{}, apperr.Internal(err)
	}
...
func (s *StoreService) ReviewPointSaving(ctx context.Context, storeID, requestID int64, req ReviewPointSavingRequest, reviewerType string, byID int64) (PointSavingView, error) {
	...
	p, err := s.repo.ReviewPointSaving(ctx, storeID, requestID, req.Decision, remark, reviewerType, byID, s.now().UTC())
	if err != nil {
		return PointSavingView{}, err
	}
	return s.pointSavingView(ctx, p), nil
...
func (s *StoreService) StaffTodayOperations(ctx context.Context, storeID int64, page httpx.Page, operationType string) (StaffTodayOperationsView, error) {
	switch operationType {
	case "", "coin_consumption", "point_deposit", "point_withdrawal":
	default:
		return StaffTodayOperationsView{}, apperr.Invalid("invalid operation type")
	}
	dayStart, dayEnd, date := s.todayBounds()
	data, err := s.repo.StaffTodayOperations(ctx, storeID, dayStart, dayEnd, operationType, page.Limit(), page.Offset())
	if err != nil {
