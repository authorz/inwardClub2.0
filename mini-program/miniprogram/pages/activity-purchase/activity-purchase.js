...
Page({
    ...
    api
      ...
      .then((res) => {
        ...
        const tickets = (a.ticketTypes || []).map((t) => {
          ...
          const limits = [purchaseLimit, maxTicketsPerOrder, Number(t.stock)].filter((value) => value > 0);
          return {
            id: t.id,
            name: t.name,
  ...
  onPickTicket(e) {
    ...
    if (ticket) this.selectTicket(ticket);
  },
  ...
  confirmPurchase() {
    ...
    const amountCent = ticket.priceCent * this.data.qty;
    this.setData({ submitting: true });
    ui.showLoading('提交中');
    api
      .createActivityOrder(
        { activityId: activity.id, ticketTypeId: ticket.id, qty: this.data.qty, amountCent, payChannel: payMethod },
        http.uuid()
      )
      .then((res) => {
